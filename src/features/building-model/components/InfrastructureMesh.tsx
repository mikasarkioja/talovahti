...
export function InfrastructureMesh({
  apartments,
}: {
  apartments: ApartmentLayout[];
}) {
  const meshRef = useRef<THREE.InstancedMesh>(null);
  const balconyApts = useMemo(
    () => apartments.filter((a) => a.balcony),
    [apartments],
  );
  ...
  return (
    <group>
      ...
      <instancedMesh
        ref={meshRef}
        args={[undefined, undefined, balconyApts.length]}
      >
