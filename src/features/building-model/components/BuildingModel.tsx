...
import {
  ...
  POI,
  ApartmentLayout,
} from "../lib/BuildingGenerator";
import { HudCard } from "@/components/ui/hud-card";
import { InstancedApartments } from "./InstancedApartments";
...
export function BuildingModel({
  ...
  const buildingLayout = useStore((state) => state.buildingLayout);
  ...
  // Generate Layout (Now using pre-calculated layout from store for performance)
  const { apartments, pois, buildingDimensions } = useMemo(() => {
    if (buildingLayout) return buildingLayout;
    return BuildingGenerator.generateLayout(
      housingCompany?.buildingConfig || undefined,
    );
  }, [buildingLayout, housingCompany?.buildingConfig]);
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-4 left-4 z-10 flex flex-col gap-2 pointer-events-none select-none">
        <HudCard className="w-auto min-w-[150px] shadow-lg">
          <div className="flex flex-col gap-1.5 text-xs">
            ...
            <div className="flex items-center gap-2">
              ...
              <span>Kiireellinen hälytys</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-2.5 h-2.5 rounded-full bg-blue-500 shadow-sm" />{" "}
              <span>Odottaa päätöstä</span>
            </div>
            {currentActiveQuarter && (
              <div className="flex items-center gap-2">
                <div className="w-2.5 h-2.5 rounded-full bg-amber-400 animate-pulse shadow-sm" />{" "}
                <span>Ajankohtainen huolto</span>
              </div>
            )}
          </div>
        </HudCard>
        {viewMode === "LIFESPAN" && (
          <HudCard className="w-auto min-w-[150px] shadow-lg">
            <div className="flex flex-col gap-1.5 text-xs">
              <div className="font-bold mb-1">Kiinteistön tekninen kunto</div>
              <div className="flex items-center gap-2">
                <div className="w-2.5 h-2.5 rounded-full bg-red-500 shadow-sm" />{" "}
                <span>Pikaista huoltoa tarvitaan</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-2.5 h-2.5 rounded-full bg-yellow-500 shadow-sm" />{" "}
                <span>Seuraa tilannetta</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-2.5 h-2.5 rounded-full bg-emerald-500 shadow-sm" />{" "}
                <span>Hyvä kunto</span>
              </div>
            </div>
          </HudCard>
        )}
        {viewMode === "VALUE_HEATMAP" && (
          <HudCard className="w-auto min-w-[150px] shadow-lg">
            <div className="flex flex-col gap-1.5 text-xs">
              <div className="font-bold mb-1">Arvon kehittyminen (PKI)</div>
              <div className="flex items-center gap-2">
                <div className="w-2.5 h-2.5 rounded-full bg-emerald-500 shadow-sm" />{" "}
                <span>Arvo säilynyt hyvin</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-2.5 h-2.5 rounded-full bg-yellow-500 shadow-sm" />{" "}
                <span>Normaali kuluminen</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-2.5 h-2.5 rounded-full bg-orange-500 shadow-sm" />{" "}
                <span>Päivitystarve lähestyy</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-2.5 h-2.5 rounded-full bg-red-500 shadow-sm" />{" "}
                <span>Välitön korjaustarve</span>
              </div>
            </div>
          </HudCard>
        )}
      ...
      <div className="absolute top-1/2 right-4 -translate-y-1/2 z-10 flex flex-col gap-2">
        <div className="bg-white/80 backdrop-blur rounded-xl p-1.5 shadow-md border border-slate-200 flex flex-col gap-1.5">
          <Button
            size="sm"
            variant={selectedFloor === null ? "primary" : "ghost"}
            className="h-11 w-11 p-0 text-[10px] font-black"
      ...
      {/* Bottom Controls */}
      <div className="absolute bottom-4 right-4 z-10 flex flex-wrap justify-end gap-3 pointer-events-auto">
        <Button
          size="sm"
          variant={viewMode === "LIFESPAN" ? "default" : "secondary"}
          className={`backdrop-blur shadow-md h-11 px-5 text-xs font-bold transition-all rounded-xl ${viewMode === "LIFESPAN" ? "bg-blue-600 hover:bg-blue-700 text-white" : "bg-white/90 text-slate-700"}`}
          onClick={() => {
        ...
        <Button
          ...
          className={`backdrop-blur shadow-md h-11 px-5 text-xs font-bold transition-all rounded-xl ${xrayEnabled ? "bg-blue-600 hover:bg-blue-700 text-white ring-2 ring-blue-400/50" : "bg-white/90 text-slate-700"}`}
          onClick={() => setXrayEnabled(!xrayEnabled)}
        >
          <Layers size={16} className="mr-2" />
          {xrayEnabled ? "Vastuu: Päällä" : "Vastuunjako"}
        </Button>
        <Button
          size="sm"
          variant="secondary"
          className="bg-white/90 backdrop-blur shadow-md h-11 px-5 text-xs font-bold rounded-xl"
          onClick={handleResetView}
        >
          <RotateCcw size={16} className="mr-2" /> Palauta
        </Button>
      </div>
