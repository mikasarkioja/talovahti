...
// Define the shader material type for JSX
declare module "@react-three/fiber" {
  interface ThreeElements {
    energyShaderMaterial: ThreeElement<typeof EnergyShaderMaterial>;
  }
}
...
export function BuildingTwin({ health = 100, onClickPoint }: BuildingProps) {
  ...
  useFrame((_state, delta) => {
    if (ref.current) {
      const uniforms = ref.current.uniforms as {
        uTime: { value: number };
        uHealth: { value: number };
      };
      uniforms.uTime.value += delta;
      uniforms.uHealth.value = health;
    }
  });
