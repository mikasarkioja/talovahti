...
export function PulseHero({
  ...
  const [pulseData, setPulseData] = useState<PulseData | undefined>(
    initialData,
  );
  ...
  const currentTemp =
    pulseData?.forecast.temp[0]?.value || weather?.temperature || 0;
  const energyImpact = Math.round(
    (21 - currentTemp) * StrategyEngine.KLOSS_CONSTANT,
  );

  if (loading)
    return (
      <div className="h-48 w-full bg-slate-100 animate-pulse rounded-xl" />
    );
        ...
        <div className="space-y-3">
          {isColdFront && (
            <motion.div
              ...
              className="flex items-center gap-3 bg-blue-500/20 border border-blue-500/30 p-3 rounded-lg"
            >
              <ThermometerSnowflake className="text-blue-300 w-6 h-6 shrink-0" />
              <div>
                <div className="font-bold text-sm text-blue-100">
                  Kylmä rintama
                </div>
                <div className="text-xs text-blue-200/80">
                  Energiankulutus +{energyImpact}%. LTV talviasetukseen.
                </div>
              </div>
            </motion.div>
          )}
            ...
            >
              ...
              <div>
                ...
                <div className="text-xs text-slate-300">
                  Auraus tilattu (Ennuste:{" "}
                  {Math.max(...(weather?.forecast.map((f) => f.snow) || []))}
                  cm).
                </div>
              </div>
            </motion.div>
