      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex items-center justify-between gap-6">
        ...
        <div className="hidden md:flex flex-1 gap-1">
          {phases.map((phase, i) => (
            ...
            >
              <div
                className={`h-1 w-full rounded-full ${i <= currentPhaseIdx ? "bg-emerald-500" : "bg-slate-800"}`}
              />
              <span
                className={`text-[10px] font-bold ${i === currentPhaseIdx ? "text-emerald-400" : "text-slate-600"}`}
              >
                {phase}
              </span>
            </div>
