...
import { CalendarPlus, Pin, Droplets, Info, Vote } from "lucide-react";
...
const MOCK_ACTIVITIES: ActivityItem[] = [
  {
    id: "1",
    title: "Vesikatko (Linjasaneeraus)",
    type: "CRITICAL",
    date: "Tänään 12:00",
    location: "A-Rappu",
  },
  {
    id: "2",
    title: "Yhtiökokouskutsu",
    type: "GOVERNANCE",
    date: "Huomenna 18:00",
  },
  {
    id: "3",
    title: "Pihakeinu asennetaan",
    type: "INFO",
    date: "Ke 10:00",
    location: "Piha",
  },
...
export function ActivityStream({
  ...
  onItemTap: (item: ActivityItem) => void;
}) {
  const { addPin } = useRenovationStore();

  const handleSwipe = (_event: unknown, info: PanInfo, item: ActivityItem) => {
...
function SwipeableCard({
  ...
  item: ActivityItem;
  onSwipe: (e: unknown, i: PanInfo, item: ActivityItem) => void;
  onTap: () => void;
}) {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case "CRITICAL":
        return <Droplets size={18} className="text-red-600" />;
      case "GOVERNANCE":
        return <Vote size={18} className="text-green-600" />;
      default:
  ...
  return (
    ...
    >
      ...
      {/* Card Front */}
      <Card
        className={`relative h-full border ${getColor(item.type)} active:cursor-grabbing`}
      >
        <CardContent className="p-4 flex flex-col gap-2 h-full justify-between">
          ...
          <div>
            ...
            {item.location && (
              <div className="text-xs opacity-80 mt-1">{item.location}</div>
            )}
          </div>
        </CardContent>
      </Card>
