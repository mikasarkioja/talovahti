...
import { CaseResolutionView } from "./CaseResolutionView";
import {
  AlertCircle,
  CheckCircle2,
  ClipboardList,
...
export function OpsKanbanBoard({ items: initialItems, userId }: OpsBoardProps) {
  ...
  const handleVerify = async (item: KanbanItem) => {
    setLoading(true);
    const result = await completeProject(item.id, userId);
    if (result.success) {
      setCompletedProjectName(item.title);
                  ...
                  col.id === "INBOX" ? (
                    ...
                    >
                      <CardHeader className="p-4 pb-2">
                        ...
                        <CardTitle className="text-sm font-bold text-brand-navy leading-tight mb-1 flex items-center justify-between">
