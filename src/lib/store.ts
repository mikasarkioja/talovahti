...
interface AppState {
  ...
  updateInvoiceStatus: (id: string, status: InvoiceStatus, approvedById?: string) => void
...
export const useStore = create<AppState>((set) => ({
  ...
  renovations: [
    { id: 'renov-hist-1', component: 'Katto', yearDone: 2010, cost: 45000, expectedLifeSpan: 40, description: 'Huopakaton uusiminen', status: 'COMPLETED' },
    { id: 'renov-plan-1', component: 'Julkisivu', plannedYear: 2027, cost: 120000, expectedLifeSpan: 30, description: 'Maalaus ja saumaus', status: 'PLANNED' },
  ],
  ...
  featureAccess: [
    { id: 'feat-1', key: 'MML_SYNC', name: 'MML Integraatio', description: 'Automaattinen tiedonsiirto.', price: 29.00, isEnabled: true },
    { id: 'feat-2', key: 'TENDERING', name: 'Kilpailutus & Valvonta', description: 'Täysi projektinhallinta.', price: 49.00, isEnabled: true },
    { id: 'feat-3', key: 'ENERGY_AI', name: 'Energia-AI & ROI', description: 'Tekoälypohjainen energiankulutuksen analyysi.', price: 15.00, isEnabled: false }
  ],
  ...
  budgetLines: [
    { id: '1', category: 'HEATING', budgetedAmount: 45000, actualSpent: 38000, year: 2026 },
  ...
  addObservation: (observation) => set((state) => ({ observations: [observation, ...state.observations] })),
  addAssessment: (observationId, assessment) => set((state) => ({ observations: state.observations.map(o => o.id === observationId ? { ...o, status: 'REVIEWED', assessment: { ...assessment, options: [] } } : o) })),
  ...
  selectWinnerBid: (projectId, tenderId, bidId) => set((state) => {
    const updatedProjects = state.projects.map(p => {
      ...
      const updatedTenders = p.tenders.map(t => t.id === tenderId ? { ...t, status: 'CLOSED' as TenderStatus, bids: t.bids.map(b => ({ ...b, isWinner: b.id === bidId })) } : t)
  ...
  addSiteReport: (report) => set((state) => ({ projects: state.projects.map(p => p.id === report.projectId ? { ...p, siteReports: [report, ...p.siteReports] } : p) })),
  updateChangeOrder: (id, status) => set((state) => ({ projects: state.projects.map(p => ({ ...p, changeOrders: p.changeOrders.map(co => co.id === id ? { ...co, status } : co) })) })),
  addMMLSyncLog: (log) => set((state) => ({ mmlSyncLogs: [log, ...state.mmlSyncLogs] })),
  addDocumentOrder: (order) => set((state) => ({ documentOrders: [order, ...state.documentOrders] })),
  setSubscriptionPlan: (plan) => set((state) => ({ subscription: { ...state.subscription, plan } })),
  toggleFeature: (key, isEnabled) => set((state) => ({ featureAccess: state.featureAccess.map(f => f.key === key ? { ...f, isEnabled } : f) })),
  addOrder: (order) => set((state) => ({ orders: [order, ...state.orders] })),
  syncInvoices: (newInvoices) => set((state) => ({ 
    invoices: [...state.invoices, ...newInvoices.filter(ni => !state.invoices.find(i => i.externalId === ni.externalId))] 
  })),
  updateInvoiceStatus: (id, status, approvedById) => set((state) => {
    ...
    return {
      invoices: state.invoices.map(i => i.id === id ? { ...i, status, approvedById: approvedById || i.approvedById } : i),
      budgetLines: updatedBudgetLines,
      feed: status === 'APPROVED' && invoice ? [
        {
  ...
  addVendorRule: (rule) => set((state) => {
      const filtered = state.vendorRules.filter(r => !(rule.yTunnus && r.yTunnus === rule.yTunnus))
      return { vendorRules: [...filtered, rule] }
  }),
  updateInvoiceCategory: (id, category) => set((state) => ({
      invoices: state.invoices.map(i => i.id === id ? { ...i, category } : i)
  }))
}))
