...
export const AccountingProvider = {
    async fetchPendingInvoices(housingCompanyId: string): Promise<PendingInvoice[]> {
        // Mock External API Call
        console.log(`[Accounting API] Fetching pending invoices for ${housingCompanyId}`)
    ...
    async approveInvoice(invoiceId: string, userId: string): Promise<boolean> {
        console.log(`[Accounting API] Approving invoice ${invoiceId} by user ${userId}`)
    ...
    async syncBankBalance(housingCompanyId: string): Promise<{ balance: number, currency: string }> {
        // Mock Bank API
        return {
            balance: 45200.50,
            currency: 'EUR'
