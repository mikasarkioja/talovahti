import { FiscalConfiguration, AnnualQuarter, TaskCategory } from '@prisma/client'
...
export class TemporalEngine {
   ...
   * @returns AnnualQuarter (Q1, Q2, Q3, Q4)
   */
  static getRelativeQuarter(date: Date, config: FiscalConfiguration): AnnualQuarter {
  ...
  static getUpcomingStatutoryDeadlines(config: FiscalConfiguration) {
    ...
    deadlines.push({
      ...
      description: 'Pidettävä 6kk kuluessa tilikauden päättymisestä (Asunto-osakeyhtiölaki).',
    ...
    financialStatementDeadline.setMonth(financialStatementDeadline.getMonth() + 4)
    ...
    deadlines.push({
      ...
      quarter: TemporalEngine.getRelativeQuarter(financialStatementDeadline, config)
    })
