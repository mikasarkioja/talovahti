...
`
  ...
  async createContractDraft(projectId: string, bidId: string) {
    ...
    await prisma.milestone.createMany({
        data: [
            { contractId: contract.id, title: "Ennakkomaksu / Materiaalit", amount: total * 0.2, dueDate: new Date(Date.now() + 7 * 86400000) },
            { contractId: contract.id, title: "Työvaihe 1: Purkutyöt", amount: total * 0.3, dueDate: new Date(Date.now() + 30 * 86400000) },
            { contractId: contract.id, title: "Työvaihe 2: Asennus", amount: total * 0.4, dueDate: new Date(Date.now() + 60 * 86400000) },
            { contractId: contract.id, title: "Vastaanottotarkastus", amount: total * 0.1, dueDate: new Date(Date.now() + 90 * 86400000) },
        ]
    })
