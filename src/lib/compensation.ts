...
export const compensationLogic = {
  async approveTask(taskId: string, approvedById: string) {
    const task = await prisma.residentTask.findUnique({
      where: { id: taskId },
    ...
    await prisma.residentTask.update({
      where: { id: taskId },
      data: { status: 'COMPLETED' }
    })

    // 2. Trigger Compensation
    if (task.rewardType === 'HOITOVASTIKE_CREDIT' && task.assignedTo?.apartmentId) {
      await this.applyInvoiceCredit(task.assignedTo.apartmentId, task.compensationAmount)
    }

    // 3. Tax Reporting Automation
    await this.checkTaxThreshold(task.assignedTo?.id, task.compensationAmount)
  ...
  async applyInvoiceCredit(apartmentId: string, amount: number) {
    ...
    console.log(`[FINANCE] Applying €${amount} credit to apartment ${apartmentId} next invoice.`)
  ...
  async generateTulorekisteriReport(userId: string, amount: number) {
    console.log(`[TAX] Generating Tulorekisteri draft for user ${userId}. Amount: ${amount} €`)
