...
import { Box, Building2, Save, Sparkles, CheckCircle2, Layers } from "lucide-react";
import { useRouter } from "next/navigation";
import { toast } from "sonner";

export function DigitalTwinWizard() {
  const { housingCompany, hydrate } = useStore();
  ...
  const handleStaircaseChange = (count: number) => {
    const newStaircases = Array.from({ length: count }, (_, i) => String.fromCharCode(65 + i));
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    const res = await saveBuildingConfig(housingCompany.id, config, generateApts);
    setLoading(false);
        ...
        <Card className="border-emerald-100 bg-emerald-50/30">
          <CardContent className="pt-6">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="font-bold text-emerald-900 text-sm">Automaattinen Ehdotus</h4>
                <p className="text-emerald-700 text-xs leading-relaxed mt-1">
                  Wizard ehdottaa <strong>{config.staircases.length * config.floors * config.unitsPerFloor}</strong> asuntoa näillä valinnoilla.
                </p>
              </div>
            </div>
          </CardContent>
      ...
      <Card className="rounded-3xl border-slate-200 shadow-xl overflow-hidden">
        <CardHeader className="bg-slate-50 border-b border-slate-100 p-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <CardTitle className="text-2xl font-black text-slate-900">Talotyypin Konfigurointi</CardTitle>
              <p className="text-slate-500 font-medium">Vaihe {step} / 3: {step === 1 ? "Perusmassa" : step === 2 ? "Kerrokset ja portaat" : "Asunnot"}</p>
            </div>
          </div>
        ...
        <CardContent className="p-8 space-y-8 min-h-[400px]">
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
              <div className="space-y-4">
                <Label className="text-sm font-black text-slate-700 flex items-center gap-2 uppercase tracking-wider">
                  <Building2 size={16} className="text-blue-500" /> Rakennuksen Muoto
                </Label>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  >
                    ...
                    <span className="text-[10px] opacity-60">Suora rakennus</span>
                  </Button>
                  <Button
                    type="button"
              ...
              {config.shape === "L" && (
                <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
                   <Label className="text-sm font-black text-slate-700 uppercase tracking-wider">Kääntöpiste</Label>
                   <Select 
                     value={config.turnAtStaircase} 
                     onValueChange={(v: string) => updateConfig({ turnAtStaircase: v })}
                   >
                     ...
                     <SelectContent>
                       {config.staircases.map(s => (
                         <SelectItem key={s} value={s}>{s}-porras</SelectItem>
                       ))}
                     </SelectContent>
                   </Select>
                   <p className="text-[10px] text-slate-400 italic">Valitse porras, jonka kohdalla rakennus kääntyy.</p>
                </div>
              )}
            </div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
               <div className="grid grid-cols-2 gap-6">
                 <div className="space-y-3">
                    ...
                    <Input 
                      ...
                      max={12} 
                      value={config.floors} 
                      onChange={(e) => updateConfig({ floors: parseInt(e.target.value) || 1 })}
                      className="rounded-xl h-12 text-lg font-bold"
                    />
                 </div>
                 <div className="space-y-3">
                    <Label className="text-xs font-black text-slate-700 flex items-center gap-2 uppercase tracking-wider">
                      <Building2 size={14} className="text-blue-500" /> Rappukäytävät
                    </Label>
                    <Input 
                      type="number" 
                      min={1} 
                      ...
                      onChange={(e) => handleStaircaseChange(parseInt(e.target.value) || 1)}
                      className="rounded-xl h-12 text-lg font-bold"
                    />
                 </div>
               </div>
               ...
               <div className="space-y-4">
                  <Label className="text-xs font-black text-slate-700 uppercase tracking-wider">Tekniset Mitat (m)</Label>
                  <div className="grid grid-cols-3 gap-4">
                     <div className="space-y-2">
                        <span className="text-[10px] text-slate-400 uppercase font-bold">Kerroskorkeus</span>
                        <Input 
                          type="number" 
                          step={0.1}
                          value={config.floorHeight} 
                          onChange={(e) => updateConfig({ floorHeight: parseFloat(e.target.value) || 3.0 })}
                          className="rounded-lg h-10 text-sm"
                        />
                     </div>
                     <div className="space-y-2">
                        <span className="text-[10px] text-slate-400 uppercase font-bold">Rungon Syvyys</span>
                        <Input 
                          type="number" 
                          step={0.1}
                          value={config.aptDepth} 
                          onChange={(e) => updateConfig({ aptDepth: parseFloat(e.target.value) || 10.0 })}
                          className="rounded-lg h-10 text-sm"
                        />
                     </div>
                     <div className="space-y-2">
                        <span className="text-[10px] text-slate-400 uppercase font-bold">Rapun Leveys</span>
                        <Input 
                          type="number" 
                          step={0.1}
                          value={config.staircaseWidth} 
                          onChange={(e) => updateConfig({ staircaseWidth: parseFloat(e.target.value) || 4.0 })}
                          className="rounded-lg h-10 text-sm"
                        />
                     </div>
                  </div>
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
               <div className="space-y-4">
                  ...
                  <Select 
                     ...
                     onValueChange={(v) => updateConfig({ unitsPerFloor: parseInt(v) })}
                   >
               ...
               <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100 space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="font-black text-slate-800 text-sm">Automaattinen Asuntolista</h4>
                      <p className="text-slate-500 text-[10px]">Luo asunnot tietokantaan automaattisesti tallennuksen yhteydessä.</p>
                    </div>
                    <Switch checked={generateApts} onCheckedChange={setGenerateApts} />
                  </div>
                  
                  {generateApts && (
                    <div className="pt-4 border-t border-slate-200 grid grid-cols-2 gap-2 max-h-[150px] overflow-y-auto pr-2 custom-scrollbar">
                       {config.staircases.map(s => (
                         Array.from({ length: config.floors * config.unitsPerFloor }, (_, i) => (
                           <div key={`${s}-${i}`} className="bg-white px-3 py-1.5 rounded-lg border border-slate-100 text-[10px] font-mono text-slate-600">
                             Asunto {s} {i + 1}
                           </div>
                         ))
                       ))}
