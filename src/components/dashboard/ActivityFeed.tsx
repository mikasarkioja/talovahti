...
export function ActivityFeed({ limit, compact }: ActivityFeedProps) {
  ...
  const activities: ActivityItem[] = [
    ...tickets.map((t) => ({
      ...
      title: isBoard
        ? `Uusi vikailmoitus: ${t.title}`
        : "Uusi vikailmoitus tehty",
      description: isBoard
        ? `Asunto ${t.apartmentId || "Yleiset"}: ${t.description}`
        : "Huoltoyhtiö on vastaanottanut ilmoituksen.",
  ...
  return (
    <div
      className={clsx(
        "bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden h-full",
                ...
                {!compact && isInitiative && (
                  <div className="mt-3 flex gap-2">
                    <button
                      onClick={() =>
                        item.initiativeId &&
                        handleJoinDiscussion(item.initiativeId)
                      }
