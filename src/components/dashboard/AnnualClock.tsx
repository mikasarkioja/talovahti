...
import { AnnualTask, AnnualQuarter } from '@prisma/client'
import { Calendar, Briefcase, FileText, CheckCircle2, AlertCircle } from 'lucide-react'

const QUARTERS: AnnualQuarter[] = ['Q1', 'Q2', 'Q3', 'Q4']
    ...
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      <PopoverTrigger asChild>
        ...
        >
          <path 
            ...
            stroke="white" 
            strokeWidth="1"
            className={cn("transition-all duration-300", isActive ? "scale-105 origin-center" : "")}
          />
          <text 
            x={labelX} 
            y={labelY} 
      ...
      <PopoverContent className="w-64 p-0" side="right">
        ...
        <div className="p-2 max-h-60 overflow-y-auto">
          {tasks.length === 0 ? (
            <div className="text-xs text-slate-500 text-center py-4">Ei tehtäviä tässä kvartaalissa.</div>
          ) : (
              ...
              {tasks.map(task => (
                ...
                >
                  <div className="font-medium mb-1 flex justify-between">
                    ...
                    {task.statutory && <Badge variant="outline" className="text-[10px] h-4 px-1">Lakisääteinen</Badge>}
                  </div>
...
export function AnnualClock() {
  ...
  const handleQuarterClick = (q: AnnualQuarter) => {
    setActiveQuarter(currentActiveQuarter === q ? null : q)
  }

  const getQuarterColor = (q: AnnualQuarter) => {
    switch (q) {
        case 'Q1': return '#3b82f6' // Blue (Winter/Spring)
        case 'Q2': return '#22c55e' // Green (Spring/Summer)
        case 'Q3': return '#eab308' // Yellow (Summer/Autumn)
        case 'Q4': return '#ef4444' // Red (Autumn/Winter - busy time)
      ...
      <CardContent className="flex flex-col items-center">
        <div className="relative w-48 h-48">
          <svg viewBox="0 0 100 100" className="w-full h-full drop-shadow-md">
            ...
            {/* Center Hub */}
            <circle cx="50" cy="50" r="15" fill="white" className="shadow-inner" />
            <text x="50" y="50" textAnchor="middle" dominantBaseline="middle" className="text-[10px] font-bold fill-slate-700">
