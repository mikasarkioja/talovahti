...
export function SolutionDesigner({
  observation,
}: {
  observation: MockObservation;
}) {
  const addSolutionOption = useStore((state) => state.addSolutionOption);
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {observation.assessment.options?.map((opt) => (
          <div
            key={opt.id}
            className="bg-white p-3 rounded border border-slate-200 shadow-sm flex justify-between items-center group"
          >
            <div>
              ...
              <div className="text-xs text-slate-500">
                ...
                {opt.lifeSpanExtension} vuotta
              </div>
            </div>
            {/* Future: Edit/Delete actions */}
          </div>
        ))}
        {(!observation.assessment.options ||
          observation.assessment.options.length === 0) &&
          !isAdding && (
