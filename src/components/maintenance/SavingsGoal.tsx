...
export function SavingsGoal({ targetAmount, targetYear, currentSavings }: { targetAmount: number, targetYear: number, currentSavings: number }) {
  ...
  return (
    <div className="bg-slate-900 text-white p-6 rounded-xl shadow-lg relative overflow-hidden">
      ...
      <div className="relative z-10">
        <h3 className="text-blue-300 font-bold uppercase tracking-wider text-xs mb-1">Rahoitustavoite {targetYear}</h3>
        ...
        <div className="flex items-center gap-3 bg-white/10 p-3 rounded-lg backdrop-blur-sm">
          ...
          <div>
            <div className="text-xs text-blue-200 uppercase font-bold">Kuukausisäästötarve</div>
            <div className="text-lg font-bold">{Math.round(monthlyNeed).toLocaleString()} € / kk</div>
          </div>
        </div>
      </div>
    </div>
