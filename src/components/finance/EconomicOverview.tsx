...
export function EconomicOverview() {
  ...
  const percentage = isBoard ? finance.collectionPercentage : 100; // Resident view usually doesn't show a generic doughnut unless we map it to something else, sticking to prompt requirements
          ...
          <div className="flex-1">
            ...
            <div className="text-2xl font-bold text-slate-900">
              {isBoard
                ...
                : `${(currentUser?.personalDebtShare || 0).toLocaleString("fi-FI")} €`}
            </div>
        ...
        <div className="p-4 bg-emerald-50 border border-emerald-100 rounded-lg flex gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-emerald-800 leading-snug">
              Tekoälyohjattu lämmitys on vähentänyt kuluja{" "}
              <strong>{finance.energySavingsPct}%</strong> tällä kvartaalilla.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 pt-2">
          <div className="p-3 bg-slate-50 rounded-lg border border-slate-100">
            ...
            <div className="font-semibold text-slate-900">
              {(finance.monthlyIncome || 0).toLocaleString("fi-FI")} €/kk
            </div>
          </div>
          <div className="p-3 bg-slate-50 rounded-lg border border-slate-100">
            <div className="flex items-center gap-2 text-slate-500 mb-1">
            ...
            <div className="font-semibold text-slate-900">
              {(finance.reserveFund || 0).toLocaleString("fi-FI")} €
            </div>
          </div>
        </div>
      </div>
