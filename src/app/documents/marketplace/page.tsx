...
export default function DocumentOrderPage() {
  ...
  const [selectedDoc, setSelectedDoc] = useState<
    "CERTIFICATE" | "ARTICLES" | "STATEMENTS" | null
  >(null);
  ...
  const products = [
    {
      ...
      price: 80.0,
      desc: "Virallinen todistus huoneiston ja yhtiön tiedoista.",
    },
    {
      id: "ARTICLES",
      label: "Yhtiöjärjestys",
      price: 25.0,
      desc: "Yhtiön säännöt ja vastikeperusteet.",
    },
    {
      id: "STATEMENTS",
      label: "Tilinpäätös & Toimintakertomus",
      price: 15.0,
      desc: "Viimeisin vahvistettu tilinpäätös.",
    },
  ] as const;
    ...
    setTimeout(() => {
      ...
      if (product) {
        ...
        if (selectedDoc === "CERTIFICATE") {
          console.log(
            "Generated:",
            generateIsannointitodistus(currentUser, finance, renovations),
          );
        }
      }
    ...
    <div className="p-8 max-w-4xl mx-auto space-y-8">
      <header>
        ...
        <p className="text-slate-500 mt-1">
          Tilaa viralliset asiakirjat heti toimitettuna. Tuotot ohjataan yhtiön
          hoitovaroihin.
        </p>
      </header>
        ...
        <div className="bg-red-50 border border-red-200 p-4 rounded-xl flex items-start gap-3">
          ...
          <div>
            <h3 className="font-bold text-red-900 text-sm">
              Varoitus: Kunnossapitosuunnitelma vanhentunut
            </h3>
            <p className="text-sm text-red-800 mt-1">
              Isännöitsijäntodistusta ei voi generoida automaattisesti, koska
              yhtiön PTS-tiedot ovat yli 12kk vanhoja. Ole hyvä ja ota yhteys
              hallitukseen.
            </p>
          </div>
        </div>
            ...
            <div className="space-y-6">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-bold">
                  {products.find((p) => p.id === selectedDoc)?.price.toFixed(2)}{" "}
                  €
                </span>
              </div>
        ...
        <div className="pt-8 border-t border-slate-200">
          ...
          <div className="bg-white rounded-lg border border-slate-200 overflow-hidden">
            {documentOrders.map((order) => (
              <div
                key={order.id}
                className="p-4 border-b border-slate-100 last:border-0 flex justify-between items-center"
              >
                <div>
                  <div className="font-bold text-slate-800 text-sm">
                    {products.find((p) => p.id === order.type)?.label}
                  </div>
