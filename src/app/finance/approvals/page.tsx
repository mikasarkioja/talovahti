...
export default function InvoiceApprovalPage() {
  ...
  const budgetLines = useStore((state) => state.budgetLines);
  const syncInvoices = useStore((state) => state.syncInvoices);
  const updateInvoiceStatus = useStore((state) => state.updateInvoiceStatus);
  const updateInvoiceCategory = useStore(
    (state) => state.updateInvoiceCategory,
  );
  const addVendorRule = useStore((state) => state.addVendorRule);
  const vendorRules = useStore((state) => state.vendorRules);
  const currentUser = useStore((state) => state.currentUser);

  const [isSyncing, setIsSyncing] = useState(false);
  const [selectedInvoice, setSelectedInvoice] = useState<MockInvoice | null>(
    null,
  );
  ...
  const handleApproval = async (invoice: MockInvoice, approved: boolean) => {
    ...
    if (invoice.externalId) {
      await accountingApi.pushApprovalStatus(
        invoice.externalId,
        newStatus,
        currentUser.id,
      );
  ...
  const handleCategoryChange = (
    ...
    if (invoice.yTunnus) {
      ...
      alert(
        `Järjestelmä oppi: ${invoice.vendorName} -> ${getCategoryLabel(newCategory)}`,
      );
    }
  };
  ...
  const getBudgetStatus = (invoice: MockInvoice) => {
    ...
    return {
      ...
      used: line.actualSpent,
      remaining,
      afterInvoice,
      isOverBudget: afterInvoice < 0,
    };
  };
      ...
      <header className="flex justify-between items-center flex-shrink-0">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Tarkasta ja hyväksy saapuneet laskut.
          </p>
        </div>
        <button
          onClick={handleSync}
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col overflow-hidden">
          ...
          <div className="p-3 border-t border-slate-100 text-center">
            <span className="text-xs text-slate-400">
              Näytä käsitellyt ({historyInvoices.length})
            </span>
          </div>
        </div>
              ...
              <div className="bg-white p-4 border-b border-slate-200 flex justify-between items-center shadow-sm z-10">
                <div>
                  ...
                  <div className="flex items-center gap-4 text-sm text-slate-500">
                    <span>
                      Eräpäivä: {selectedInvoice.dueDate.toLocaleDateString()}
                    </span>
                    <span className="text-slate-300">|</span>
                    <span>
                      Vaatii hyväksynnän:{" "}
                      {getRequiredApprovers(selectedInvoice).join(", ")}
                    </span>
                  </div>
                </div>
                ...
                {currentUser?.canApproveFinance ? (
                  <div className="flex gap-3">
                    <button
                      onClick={() => handleApproval(selectedInvoice, false)}
                      className="px-4 py-2 bg-red-50 text-red-700 hover:bg-red-100 border border-red-200 rounded-lg font-bold flex items-center gap-2 transition-colors"
                    >
                      <XCircle size={18} /> Hylkää
                    </button>
                    <button
                      onClick={() => handleApproval(selectedInvoice, true)}
                      className="px-6 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg font-bold flex items-center gap-2 shadow-md transition-colors"
                    >
                      <CheckCircle2 size={18} /> Hyväksy
                    </button>
                  </div>
                ) : (
                  <div className="text-xs font-bold text-slate-400 bg-slate-100 px-3 py-2 rounded">
                    Vain lukuoikeus
                  </div>
                )}
              </div>
                ...
                <div className="w-full md:w-80 bg-white border-l border-slate-200 p-6 overflow-y-auto">
                  <div className="mb-6">
                    ...
                    {(() => {
                      ...
                      if (
                        match.confidence < 100 &&
                        match.reason !== "RULE_MATCH"
                      )
                        return (
                          <div className="text-xs text-purple-600 mt-1 flex items-center gap-1">
                            <Brain size={12} /> Ehdotus:{" "}
                            {getCategoryLabel(match.category)} (
                            {match.confidence}%)
                          </div>
                        );
                      if (match.reason === "RULE_MATCH")
                        return (
                          <div className="text-xs text-green-600 mt-1 flex items-center gap-1">
                            <CheckCircle2 size={12} /> Opetettu sääntö
                          </div>
                        );
                      return null;
                    })()}
                  ...
                  {(() => {
                    ...
                    if (
                      ...
                      check.used === undefined ||
                      check.totalBudget === undefined
                    )
                      return (
                    ...
                    const percentAfter =
                      ((check.used + selectedInvoice.amount) /
                        check.totalBudget) *
                      100;
                          ...
                          <div className="relative h-4 bg-slate-200 rounded-full overflow-hidden mb-1">
                            ...
                            <div
                              ...
                              style={{
                                ...
                                width: `${Math.min(percentAfter - percentUsed, 100 - percentUsed)}%`,
                              }}
                            ></div>
                          </div>
