...
import {
  analyzeBudgetAccuracy,
  calculateVastikeHealth,
    ...
    <div className="p-8 max-w-6xl mx-auto space-y-8">
      <header>
        ...
        <p className="text-slate-500 mt-1">
          Reaaliaikainen budjetin seuranta ja vastikeoptimointi.
        </p>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col items-center">
          <h2 className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-6 w-full text-left">
            Vastike-O-Meter™
          </h2>
          ...
          <tbody className="divide-y divide-slate-100">
            {variances.map((row) => (
              ...
              >
                ...
                <td className="px-6 py-4 text-right">
                  <span
                    className={clsx(
                      ...
                      Math.abs(row.relativeVariance) < 5
                        ? "bg-slate-100 text-slate-600"
                        : row.relativeVariance > 0
                          ? "bg-red-100 text-red-700"
                          : "bg-green-100 text-green-700",
                    )}
                  >
                    {row.relativeVariance > 0 ? "+" : ""}
                    {row.relativeVariance.toFixed(1)}%
                  </span>
                </td>
                <td
                  className={clsx(
                    "px-6 py-4 text-right font-bold",
                    row.absoluteVariance > 0
                      ? "text-red-600"
                      : row.absoluteVariance < 0
                        ? "text-green-600"
                        : "text-slate-400",
                ...
                >
                  {row.absoluteVariance > 0 ? "+" : ""}
                  {row.absoluteVariance.toLocaleString()} €
                </td>
              </tr>
            ))}
          </tbody>
