...
export async function getPulseData(companyId: string): Promise<PulseData> {
  ...
  try {
    const response = await fetch(url, {
      next: { tags: [`pulse-${companyId}`], revalidate: 1800 },
    });
...
export async function refreshPulse(companyId: string) {
  // Use revalidatePath as a safer alternative if revalidateTag has issues in this env
