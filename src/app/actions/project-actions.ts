...
export async function completeProjectAction(projectId: string, userId: string) {
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      const newHealth = await HealthScoreEngine.recalculateBuildingHealth(project.housingCompanyId, tx);
      ...
      const boardProfile = await tx.boardProfile.upsert({
        ...
        create: {
          ...
          achievements: [{
            ...
            name: "Hanke valmis",
            description: "Hallitus on vienyt rakennushankkeen onnistuneesti loppuun.",
            date: new Date()
          }]
        },
        update: {
      ...
      await tx.auditLog.create({
        data: {
          ...
          metadata: {
            ...
            message: "Vastaanottotarkastus suoritettu. Sopimus siirretty takuuvaiheeseen ja hanke arkistoitu."
          }
        }
      });
