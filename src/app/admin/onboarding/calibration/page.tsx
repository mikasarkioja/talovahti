...
import { ExtruderEngine } from "@/features/building-model/lib/ExtruderEngine";
...
export default function CalibrationPage() {
  ...
  const handleSave = () => {
    ...
    const worldPoints = points.map((p) =>
      ExtruderEngine.map2DTo3D(p[0], p[1], 0.05),
    ); // 1px = 5cm
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="text-sm text-slate-500">
        <p>
          Ohje: Lataa pohjakuva ja klikkaa kulmapisteet kiertäen myötäpäivään.
        </p>
      </div>
    </div>
  );
