...
export default function BillingPage() {
        ...
        <header className="flex justify-between items-center">
          <div>
            ...
            <p className="text-slate-400 mt-1">
              System-wide revenue and usage metrics.
            </p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="bg-white/5 p-6 rounded-xl border border-white/10">
            ...
            <div className="text-3xl font-bold text-white">
              {systemStats.mrr.toLocaleString()} €
            </div>
          ...
          <div className="bg-white/5 p-6 rounded-xl border border-white/10">
            ...
            <div className="text-3xl font-bold text-white">
              {systemStats.totalTransactionRevenue.toLocaleString()} €
            </div>
          ...
          <div className="bg-white/5 p-6 rounded-xl border border-white/10">
            ...
            <div className="text-3xl font-bold text-white">
              {(
                systemStats.certificatesSold / systemStats.activeCompanies
              ).toFixed(1)}
            </div>
            ...
            {[1, 2, 3].map((i) => (
              <div key={i} className="p-4 flex justify-between text-sm">
                <span className="text-slate-300">
                  As Oy Example {i} - Pro Subscription
                </span>
  ...
  // Board View
  const currentPlan =
    PRICING_TIERS[subscription.plan as keyof typeof PRICING_TIERS];
      ...
      <header className="flex justify-between items-start">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Hallinnoi taloyhtiön lisenssiä ja maksutapoja.
          </p>
        </div>
        {canSeeAdmin && (
          <button
          ...
          <div className="space-y-3">
            ...
            {[1, 2, 3].map((i) => {
              ...
              return (
                <div
                  key={i}
                  className="flex justify-between items-center text-sm p-2 hover:bg-slate-50 rounded transition-colors cursor-pointer group"
                >
