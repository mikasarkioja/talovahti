...
export default async function CertificatePage(props: {
  ...
  const userQuery = typeof searchParams.user === "string" ? searchParams.user : undefined;
    ...
    return (
      <div className="p-20 text-center space-y-4">
        ...
        <p className="text-slate-500">
          {!isBoard ? "Todistuksen katselu vaatii onnistuneen tilauksen ja maksun." : "Vain hallituksen jäsenillä on oikeus generoida isännöitsijäntodistus."}
        </p>
      </div>
    );
  }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mt-10">
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-2 text-sm">
                <span className="text-slate-500 font-bold uppercase text-[10px]">Nimi</span>
                <span className="col-span-2 font-bold text-slate-900">{company.name}</span>
                
                <span className="text-slate-500 font-bold uppercase text-[10px]">Y-tunnus</span>
                <span className="col-span-2 font-medium">{company.businessId}</span>
                
                <span className="text-slate-500 font-bold uppercase text-[10px]">Osoite</span>
                <span className="col-span-2 font-medium">{company.address}, {company.postalCode} {company.city}</span>
                
                <span className="text-slate-500 font-bold uppercase text-[10px]">Tontti</span>
                <span className="col-span-2 font-medium uppercase text-[11px]">Oma</span>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-2 text-sm">
                <span className="text-slate-500 font-bold uppercase text-[10px]">Maksuvalmius</span>
                <span className="col-span-2 font-bold text-emerald-700">
                  {(company.healthScore || 0) > 80 ? 'ERINOMAINEN' : 'TYYDYTTÄVÄ'}
                </span>
                
                <span className="text-slate-500 font-bold uppercase text-[10px]">Kassavaranto</span>
                <span className="col-span-2 font-bold">{cashBalance.toLocaleString('fi-FI')} €</span>
                
                <span className="text-slate-500 font-bold uppercase text-[10px]">Vastikejäämät</span>
                <span className="col-span-2 font-bold text-emerald-600">0,00 € (Vastikevalvonta OK)</span>
                
                <span className="text-slate-500 font-bold uppercase text-[10px]">Lainaosuudet</span>
                <span className="col-span-2 font-medium">Laskettu huoneistokohtaisesti</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-8 md:p-12 space-y-12">
          ...
          <section>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-black text-slate-900 uppercase tracking-tight">Kiinteistön Kuntoindeksi</h2>
              <div className="text-sm font-bold text-slate-400 uppercase tracking-widest">Health Score</div>
            </div>
            <Card className="border-2 border-slate-100 bg-white shadow-none">
              <CardContent className="p-6 flex flex-col md:flex-row justify-between items-center gap-6">
                <div className="space-y-2">
                  <p className="text-sm text-slate-600 leading-relaxed font-medium">
                    Kuntoindeksi perustuu rakennusteknisiin havaintoihin, suoritettuihin perusparannuksiin sekä yhtiön taloudelliseen vakauteen. 
                    Luku päivittyy automaattisesti jokaisen asiantuntijatarkastuksen ja hyväksytyn laskun myötä.
                  </p>
                  <div className="flex gap-2">
                    <Badge variant="outline" className="text-[10px] font-bold border-slate-200">Tekninen: {company.healthScoreTechnical}/100</Badge>
                    <Badge variant="outline" className="text-[10px] font-bold border-slate-200">Talous: {company.healthScoreFinancial}/100</Badge>
                  </div>
                </div>
                <div className="flex flex-col items-center shrink-0">
                  <div className="text-6xl font-black text-blue-900 tabular-nums">
                    {company.healthScore}<span className="text-2xl text-slate-300 ml-1">/100</span>
                  </div>
                  <div className="mt-1 text-[10px] font-black text-blue-900/40 uppercase tracking-widest">Status: Optimaalinen</div>
                </div>
              </CardContent>
            </Card>
          </section>
          ...
          <section>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-slate-50 hover:bg-slate-50 border-none">
                  <TableHead className="text-[10px] font-black uppercase text-slate-500">Valmistumisvuosi</TableHead>
                  <TableHead className="text-[10px] font-black uppercase text-slate-500">Toimenpide / Hanke</TableHead>
                  <TableHead className="text-[10px] font-black uppercase text-slate-500">Sopimusmalli</TableHead>
                  <TableHead className="text-right text-[10px] font-black uppercase text-slate-500">Kustannus</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                  ...
                  company.projects.map((project) => (
                    <TableRow key={project.id} className="border-slate-100">
                      ...
                      <TableCell className="text-right font-bold tabular-nums">
                        {project.estimatedCost ? `${project.estimatedCost.toLocaleString('fi-FI')} €` : '-'}
                      </TableCell>
                    </TableRow>
                  ))
                )}
            ...
            <p className="mt-4 text-[10px] text-slate-400 italic font-medium">
              * Lista sisältää vain Talovahti-järjestelmän kautta loppuunsaatetut ja arkistoidut hankkeet.
            </p>
          </section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-end">
              <div className="space-y-4">
                ...
                <p className="text-[10px] text-slate-400 leading-relaxed max-w-sm">
                  Tämä asiakirja on generoitu automaattisesti perustuen taloyhtiön reaaliaikaiseen dataan. 
                  Kaikki muutokset datassa kirjataan hallituksen päätöshistoriaan.
                </p>
              </div>
              <div className="text-right space-y-1">
                <div className="font-mono text-[9px] text-slate-300 uppercase tracking-widest">
                  Verification ID: {crypto.randomUUID().split('-')[0].toUpperCase()}
                </div>
                <div className="font-mono text-[9px] text-slate-300 uppercase tracking-widest">
                  Audit Log Ref: {new Date().getTime().toString(16).toUpperCase()}
                </div>
              </div>
            </div>
          </div>
