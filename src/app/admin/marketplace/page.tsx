...
export default function MarketplacePage() {
  const featureAccess = useStore((state) => state.featureAccess);
  ...
  const handleToggleFeature = (key: string, enabled: boolean) => {
    ...
    if (!enabled) {
      ...
      if (
        confirm(
          `Osta lisäosa hintaan ${featureAccess.find((f) => f.key === key)?.price} €/kk?`,
        )
      ) {
    ...
    <div className="p-8 max-w-6xl mx-auto space-y-12">
      <header className="text-center max-w-2xl mx-auto">
        <h1 className="text-4xl font-bold text-slate-900 mb-4">
          Laajenna Taloyhtiö OS:n mahdollisuuksia
        </h1>
        <p className="text-lg text-slate-500">
          Valitse tarpeisiinne sopiva palvelutaso tai täydennä kokonaisuutta
          älykkäillä lisäosilla.
        </p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div
          className={clsx(
            "p-8 rounded-2xl border-2 flex flex-col",
        ...
        >
          ...
          <div className="text-4xl font-bold text-slate-900 mb-6">
            0 €{" "}
            <span className="text-base font-normal text-slate-500">/ kk</span>
          </div>
          <ul className="space-y-3 mb-8 flex-1">
            <li className="flex items-center gap-2 text-sm text-slate-600">
              <Check size={16} className="text-green-500" /> Digitaalinen
              kaksonen
            </li>
            <li className="flex items-center gap-2 text-sm text-slate-600">
              <Check size={16} className="text-green-500" /> Hallituksen työkalu
            </li>
            <li className="flex items-center gap-2 text-sm text-slate-600">
              <Check size={16} className="text-green-500" /> Perusviestintä
            </li>
          </ul>
          <button
            disabled
            className="w-full py-3 rounded-lg font-bold bg-slate-200 text-slate-500 cursor-not-allowed"
          >
            Nykyinen Taso
          </button>
        </div>
        ...
        <div
          className={clsx(
            "p-8 rounded-2xl border-2 flex flex-col relative",
        ...
        >
          ...
          <ul className="space-y-3 mb-8 flex-1">
            <li className="flex items-center gap-2 text-sm text-slate-600">
              <Check size={16} className="text-green-500" /> Kaikki
              Basic-ominaisuudet
            </li>
            <li className="flex items-center gap-2 text-sm text-slate-600">
              <Check size={16} className="text-green-500" /> Isännöintityökalut
            </li>
            <li className="flex items-center gap-2 text-sm text-slate-600">
              <Check size={16} className="text-green-500" /> MML Integraatio
            </li>
            <li className="flex items-center gap-2 text-sm text-slate-600">
              <Check size={16} className="text-green-500" /> Rajoittamaton
              tallennustila
            </li>
          </ul>
          <button
        ...
        >
          ...
          <ul className="space-y-3 mb-8 flex-1">
            ...
            <li className="flex items-center gap-2 text-sm text-slate-600">
              <Check size={16} className="text-green-500" /> Projektinjohto
              (Isännöitsijätön)
            </li>
            <li className="flex items-center gap-2 text-sm text-slate-600">
              <Check size={16} className="text-green-500" /> Energia-AI
              optimointi
            </li>
            <li className="flex items-center gap-2 text-sm text-slate-600">
              <Check size={16} className="text-green-500" /> Oma asiakasvastaava
            </li>
          </ul>
          <button
            ...
            >
              ...
              <div className="flex items-center justify-between mt-auto">
                ...
                <button
                  ...
                  className={clsx(
                    "text-sm font-bold px-4 py-2 rounded-lg transition-colors border",
                    feature.isEnabled
