...
export default function LibraryPage() {
  ...
  const handleGenerate = () => {
    ...
    const decisionItems = initiatives.filter(
      (i) =>
        i.status === "VOTING" ||
        i.status === "APPROVED" ||
        i.status === "REJECTED",
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="space-y-4">
          ...
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm hover:shadow transition-shadow cursor-pointer"
            >
          ...
          {minutes ? (
            ...
            <div className="h-[600px] border-2 border-dashed border-slate-200 rounded-xl flex items-center justify-center text-slate-400">
              <div className="text-center">
                ...
                <p>
                  Valitse &ldquo;Luo pöytäkirja&rdquo; generoidaksesi uuden
                  luonnoksen.
                </p>
              </div>
            </div>
