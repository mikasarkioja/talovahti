...
import { History, Wrench, Calendar, AlertTriangle, ArrowRight } from "lucide-react";
...
export default function MaintenanceHistoryPage() {
  const { renovations, finance, currentUser, observations } = useStore()
  ...
  const prioritizedPlan = planned.map(item => {
    ...
    <div className="p-8 max-w-6xl mx-auto space-y-8">
      <header>
        ...
        <p className="text-slate-500 mt-1">
          Kiinteistön elinkaaren hallinta ja pitkän tähtäimen kunnossapitosuunnitelma (PTS).
        </p>
      </header>
                    ...
                    <div className="p-4 flex justify-between items-start">
                       <div className="flex gap-3">
                         <div className="flex flex-col items-center">
                           ...
                           <span className={clsx("text-[10px] font-bold px-1.5 py-0.5 rounded border", item.priority.color)}>
                             {item.priority.label}
                           </span>
                         </div>
                         <div>
                           <h4 className="font-bold text-slate-900 text-lg">{item.component}</h4>
                           <p className="text-sm text-slate-600 line-clamp-2">{item.description}</p>
                         </div>
                       </div>
                       <div className="text-right">
                         <div className="text-lg font-bold text-slate-900">
                           ~{(Math.round(estimatedCost / 1000) * 1000).toLocaleString()} €
                         </div>
                         {!isFunded && (
                           <div className="text-xs text-red-600 font-medium flex items-center justify-end gap-1 mt-1">
                             <AlertTriangle size={12} />
                             Rahoitusvaje: {(estimatedCost - finance.reserveFund).toLocaleString()} €
                           </div>
                         )}
                       </div>
                    </div>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-slate-50 p-6 rounded-xl border border-slate-200">
             ...
             <p className="text-sm text-slate-600 mb-4">
               Kaikki pienkorjaukset ja huoltotoimenpiteet kirjataan digitaaliseen huoltokirjaan.
             </p>
