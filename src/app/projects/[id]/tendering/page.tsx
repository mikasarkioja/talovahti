...
import { AlertTriangle, CheckCircle, TrendingUp, Download, Send, BarChart3 } from 'lucide-react'
...
export default function TenderingPage({ params }: { params: { id: string } }) {
  ...
  const scoredBids = bids.map(bid => {
    ...
    const totalScore = (priceScore * 0.5) + (bid.roiImpact * 0.3) + (riskComponent * 0.2)
  ...
  const handleApprove = () => {
    alert(`Valinta hyväksytty: ${optimalBid.contractor}.\n\nSiirrytään hallituksen äänestykseen.`)
  }

  const handleSendRFQ = () => {
    alert('Tarjouspyyntö (RFQ) lähetetty 5 valitulle kumppanille.\n\nLiitteet: 3D-energiaskannaus, Kuntoarvio.')
  }
  ...
  return (
    <div className="space-y-6 p-6 max-w-7xl mx-auto">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Kilpailutus & Vertailu</h1>
          <p className="text-slate-500">Ikkunaremontti (ID: {params.id})</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card className="bg-slate-50 border-slate-200">
            <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-slate-500">Takaisinmaksuaika</CardTitle>
            </CardHeader>
            <CardContent>
                <div className="text-2xl font-bold text-slate-900">{PROJECT_ROI.payback} Vuotta</div>
            </CardContent>
        </Card>
        <Card className="bg-slate-50 border-slate-200">
            <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-slate-500">10v NPV (Nykyarvo)</CardTitle>
            </CardHeader>
            <CardContent>
                <div className="text-2xl font-bold text-emerald-600">+{PROJECT_ROI.npv.toLocaleString()} €</div>
            </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
            ...
            <CardDescription>Järjestetty painotetun pisteytyksen mukaan (Hinta 50%, Laatu 30%, Riski 20%)</CardDescription>
        </CardHeader>
        <CardContent>
                ...
                <table className="w-full text-sm text-left">
                    ...
                    <tbody>
                        {scoredBids.map((bid, index) => (
                            <tr key={bid.id} className={`border-b last:border-0 ${index === 0 ? 'bg-emerald-50/50' : 'hover:bg-slate-50'}`}>
                                <td className="p-4">
                                    <div className="font-medium">{bid.contractor}</div>
                                    {index === 0 && <Badge className="mt-1 bg-emerald-600">Optimaalinen</Badge>}
                                </td>
                                <td className="p-4 text-base">{bid.price.toLocaleString()} €</td>
                                <td className="p-4">
                                    <div className="flex items-center gap-2">
                                        <span className="font-bold">{Math.round(bid.totalScore)}</span>
                                ...
                                <td className="p-4">
                                    {bid.isRisk ? (
                                        ...
                                        <Badge variant="outline" className="bg-emerald-100 text-emerald-700 border-emerald-200">Matala</Badge>
                                    )}
                                    {bid.price < avgPrice * 0.8 && <div className="text-xs text-red-500 mt-1">Poikkeuksellisen halpa</div>}
                                </td>
            ...
            <CardContent className="h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={scoredBids}>
                        <XAxis dataKey="contractor" tick={{fontSize: 10}} interval={0} />
                        <YAxis />
                        <Tooltip />
            ...
            <CardContent className="h-[300px]">
                 <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={scoredBids} layout="vertical">
                        ...
                        <YAxis dataKey="contractor" type="category" width={100} tick={{fontSize: 10}} />
                        <Tooltip />
                        <Bar dataKey="riskScore" name="Riski-indeksi">
                             {scoredBids.map((entry, index) => (
                                <Cell key={`cell-${index}`} fill={entry.isRisk ? '#ef4444' : '#3b82f6'} />
                            ))}
                        </Bar>
                    </BarChart>
                </ResponsiveContainer>
